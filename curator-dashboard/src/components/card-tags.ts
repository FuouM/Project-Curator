...
export function renderIdentityListHtml(
  names: { name: string }[] | undefined,
  style = "",
): SafeHtml {
  if (!names || names.length === 0) return "" as SafeHtml;
  const styleAttr = style ? ` style="${style}"` : "";
  return html`<div class="identity-list" ${styleAttr}>
    ${names.map((ci) => `<span class="tag-pill tag-identity"><i class="bi bi-person-fill"></i> ${ci.name}</span>`).join("")}
  </div>`;
}
...
export function renderUserTagsListHtml(tags: TagSummary[] | undefined, style = ""): SafeHtml {
  ...
  const styleAttr = style ? ` style="${style}"` : "";
  return html`<div class="user-tags-list" ${styleAttr}>
    ${tags.map((t) => getTagPillHtml(t)).join("")}
  </div>`;
}

export function renderCardTagsContainerHtml(
  img: { tags: TagSummary[] },
  isFeatured = false,
): SafeHtml {
  const tags = img.tags || [];
  ...
  const userTags = tags.filter((t) => t.category === "user" || t.source_name === "user");
  const aiTags = tags.filter((t) => !(t.category === "user" || t.source_name === "user"));
  ...
  return html`
    ...
    <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 6px; width: 100%;">
      <div
        style="font-family: monospace; font-size: 10px; font-weight: bold; background-color: var(--sys-control-bg, #fff); border: 1px solid var(--sys-border-dark, #b0b0b0); border-radius: 2px; padding: 2px 6px; color: var(--sys-text-secondary, #666); text-transform: uppercase; letter-spacing: 0.5px; line-height: 1; white-space: nowrap;"
      >
        ${taggerLabel}
      </div>
      ...
      <button
        type="button"
        class="tag-copy-btn"
        data-action="copy-tags"
        data-copy-tags="${copyText}"
        title="Copy tags to clipboard"
      >
        <i class="bi bi-clipboard"></i>
      </button>
    </div>
    <div class="tag-list" style="margin-top: 0;">${tagHtml}</div>
  `;
}
...
export function renderParsedMetadataHtml(meta: ParsedMetadata): SafeHtml {
  ...
  if (meta.match_type === "anime_screenshot") {
    ...
    const partialBadge = meta.partial
      ? ' <span style="font-size: 9px; color: #856404;">(partial)</span>'
      : "";
    parts.push(
      `<span class="tag-pill tag-copyright" style="font-size: 10px; font-weight: 600;"><i class="bi bi-film"></i> Anime Screenshot: ${animeName} - ${epNum}${partialBadge}</span>`,
    );
    ...
    const hashLink = hash
      ? `<a href="https://danbooru.donmai.us/posts?tags=md5%3A${hash}" target="_blank" rel="noopener" style="color: inherit; text-decoration: underline; font-size: inherit;" onmouseover="this.style.textDecoration='none'" onmouseout="this.style.textDecoration='underline'" title="${hash}">${hashDisplay}</a>`
      : "?";
    parts.push(
      `<span class="tag-pill tag-copyright" style="font-size: 10px; font-weight: 600;"><i class="bi bi-grid-3x3-gap"></i> ${artist} - ${hashLink}</span>`,
    );
  } else {
    if (meta.datetime_iso) {
      parts.push(
        `<span class="tag-pill tag-meta" style="font-size: 10px;"><i class="bi bi-clock"></i> 4chan: ${meta.datetime_iso}</span>`,
      );
    } else if (meta.artist) {
      parts.push(
        `<span class="tag-pill tag-artist" style="font-size: 10px;"><i class="bi bi-person"></i> artist: ${meta.artist}</span>`,
      );
    }
  }
  ...
  if (meta.pixiv_id) {
    ...
    parts.push(
      `<span class="tag-pill tag-copyright" style="font-size: 10px;"><i class="bi bi-image"></i> pixiv: <a href="https://www.pixiv.net/en/artworks/${meta.pixiv_id}" target="_blank" rel="noopener" style="color: inherit; text-decoration: underline; font-size: inherit;" onmouseover="this.style.textDecoration='none'" onmouseout="this.style.textDecoration='underline'">${meta.pixiv_id}</a>${pageStr}</span>`,
    );
  }
  if (meta.twitter_id)
    parts.push(
      `<span class="tag-pill tag-character" style="font-size: 10px;"><i class="bi bi-twitter"></i> twitter: ${meta.twitter_id}</span>`,
    );
  ...
  for (const tag of meta.extracted_tags) {
    ...
    if (tag.startsWith("artist:") || tag.startsWith("pixiv:") || tag.startsWith("twitter:"))
      continue;
    ...
    parts.push(
      `<span class="tag-pill ${tagClass}" style="font-size: 10px; font-family: monospace;">${tag}</span>`,
    );
  }
