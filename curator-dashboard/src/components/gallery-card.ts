...
export function renderOcrBlockHtml(ocrText: string): SafeHtml {
  return html`
    <div class="ocr-block" data-action="toggle-ocr">
      <i class="bi bi-file-earmark-text ocr-icon" data-action="copy-ocr" data-ocr-copy="${ocrTextAttr(ocrText)}" title="Copy OCR text"></i>
      <span class="ocr-block-text">${ocrText.replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\n/g, '<br>')}</span>
    </div>
  `;
}
...
export function renderGalleryCardHtml(img: GalleryCardViewData): SafeHtml {
  ...
  return html`
    <div class="image-card ${img.isSelected ? 'selected' : ''} ${img.isLucky ? 'lucky-highlight' : ''} ${sizeClass}" style="${styleAttributes}" data-image-id="${img.id}" data-filepath="${img.filepath}">
      <input type="checkbox" class="card-select-checkbox" data-id="${img.id}" ${img.isSelected ? 'checked' : ''} />
      ...
      <div class="${previewClass}" ${img.width && img.height ? `style="aspect-ratio: ${img.width} / ${img.height};"` : ''}>
        <img data-thumb-id="${img.id}" data-mtime="${img.mtime || 0}" data-filepath="${img.filepath}" data-is-video="${img.video ? '1' : '0'}" data-pending="${isPending ? '1' : '0'}" ${img.keepLoaded ? 'data-keep-loaded="1"' : ''} ${srcAttr} alt="Image Preview" style="width: 100%; height: 100%; object-fit: cover;" class="${imgClass}" />
      ...
      <div class="image-info">
        ...
        <div style="display: flex; gap: 4px; margin-top: auto; width: 100%;">
          <button class="win-button" style="font-size: 11px; flex: 1;" data-action="open-tags" data-id="${img.id}" data-filepath="${escapeAttr(img.filepath)}">
            <i class="bi bi-tag"></i> Tags
          </button>
          <button class="win-button" style="font-size: 11px; flex: 1;" data-action="find-similar" data-filepath="${escapeAttr(img.filepath)}">
...
export const meta: ComponentMeta = {
  name: "Gallery Card",
  description: "Grid visual cards optimized for lazy loaded assets and fast aspect-ratio layout grids.",
  variants: [
    {
      name: "Ready & Active States",
      render: () => renderGalleryCardHtml({
        id: 10,
        ...
        isMissing: false,
        isLucky: false,
        cachedThumbSrc: "data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='100' height='100'><rect width='100' height='100' fill='%23ccc'/></svg>",
        width: 800,
        height: 600,
        parsedMetadata: {
          match_type: "danbooru",
    ...
    },
    {
      ...
      render: () => renderGalleryCardHtml({
        ...
        isMissing: false,
        isLucky: true,
        cachedThumbSrc: "data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='100' height='100'><rect width='100' height='100' fill='%23666'/></svg>",
        width: 1920,
        height: 1080,
