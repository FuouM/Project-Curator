...
export function renderImageCard(
  ...
  const extraTagHtml =
    extraCount > 0 ? `<span class="tag-pill tag-pill-overflow">+${extraCount} more</span>` : "";
  ...
  return html`
    <div class="image-card">
      ...
      <div class="image-info">
        ...
        <div class="tag-list">${tagPills}${extraTagHtml}</div>
      </div>
    </div>
  `;
}
  ...
  variants: [
    {
      ...
      render: () => html`
        <div
          style="display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 12px; width: 100%;"
        >
