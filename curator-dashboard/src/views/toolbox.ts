...
function loadImage(path: string) {
  ...
  probe.onerror = () => {
    ...
    if (sourceStatus)
      setStatusMessage(sourceStatus, "Could not load image from that path.", "error");
  };
...
async function runTagging() {
  ...
  try {
    const resp = await typedCall(
      ...
      EphemeralTagImageRequestSchema,
      {
...
async function runOcr() {
  ...
  try {
    ...
    if (currentOcr.length === 0) {
      ocrList.innerHTML =
        "<span style='color: #64748b; font-style: italic; font-size: 11px;'>No text detected.</span>";
  ...
  const rects = currentDetections
    .map((d) => {
      const color =
        d.identity_id !== null
          ? IDENTITY_COLORS[
              (((d.identity_id - 1) % IDENTITY_COLORS.length) + IDENTITY_COLORS.length) %
...
async function runDetection() {
  ...
  try {
    ...
    if (currentDetections.length === 0) {
      detList.innerHTML =
        "<span style='color: #64748b; font-style: italic; font-size: 11px;'>No persons detected.</span>";
...
function renderSafetyHtml(s: {
  ...
  const bar = (label: string, icon: string, value: number): string => {
    ...
    return (
      ...
      '<span style="width:52px;font-weight:600;">' +
      label +
      "</span>" +
      '<div class="prob-bar" style="flex:1;min-width:0;"><div style="height:100%;width:' +
      pct +
      '%;background:var(--sys-accent,#0078d7);"></div></div>' +
      '<span style="width:40px;text-align:right;color:#333;">' +
      pct +
      "%</span>" +
      "</div>"
    );
  ...
  return (
    ...
    col(
      bar("Safe", "bi-shield-check", s.safe_score ?? 0) +
        bar("Drawing", "bi-palette", s.drawing_score ?? 0),
    ) +
    col(
...
function setSafetySummary(ns: number, sf: number) {
  ...
  summary.innerHTML =
    ...
    ((ns * 1000) / 10).toFixed(1) +
    "%</span>" +
    '<span style="color:#2e7d32;margin-left:10px;">SFW ' +
    ((sf * 1000) / 10).toFixed(1) +
    "%</span>";
...
export function renderToolboxHtml(): SafeHtml {
  return html`
    <div class="toolbox-layout">
      ...
      <div class="toolbox-column">
        <div class="group-box" id="toolbox-source-group">
          ...
          <div class="form-group" style="margin-top: 6px;">
            <div class="input-wrapper" style="flex: 1;">
              <input
                ...
                id="toolbox-image-path-input"
                placeholder="Select or paste a path to any image file..."
              />
          ...
          <p
            id="toolbox-source-status"
            style="font-size: 11px; color: var(--sys-text-subtle); min-height: 16px; margin: 6px 0 0 0;"
          >
            Drag &amp; drop an image here, or pick any image file. Processing is ephemeral — nothing
            is written to your library.
          </p>
          ...
          <div class="toolbox-preview-box" id="toolbox-preview-group" style="display: none;">
            <div
              style="display: flex; justify-content: space-between; align-items: center; gap: 8px; margin-top: 8px; margin-bottom: 8px;"
            >
              <div
                id="toolbox-preview-filename"
                title=""
                style="font-size: 11px; color: var(--sys-text-subtle); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; flex: 1;"
              ></div>
              <button
                type="button"
                class="win-button"
                id="toolbox-fullscreen-btn"
            ...
            <div
              class="toolbox-preview-scroll"
              style="background: #1a1a1a; border: 1px solid var(--sys-border-dark);"
            >
              <svg
                id="toolbox-preview-svg"
      ...
      <div class="toolbox-column">
        <div class="group-box toolbox-fixed">
          ...
          <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
            <span style="font-size: 11px; color: var(--sys-text-subtle); flex: 1;"
              >Find similar images in your library using the current image as the query.</span
            >
        ...
        <div class="group-box toolbox-panel">
          ...
          <div class="form-group" style="margin-top: 6px; flex-wrap: wrap;">
            <select
              class="input-field"
              id="toolbox-tagger-select"
              style="width: 155px; font-size: 11px;"
              title="Tagger model to use for this run"
            >
            ...
            <select
              class="input-field"
              id="toolbox-tagger-threshold-select"
              style="width: 155px; font-size: 11px;"
            >
            ...
            <button type="button" class="win-button" id="toolbox-tag-btn">
              <i class="bi bi-stars"></i> Tag Image
            </button>
            <button
              type="button"
              class="win-button"
              id="toolbox-copy-tags-btn"
              style="display: none;"
            >
              <i class="bi bi-clipboard"></i> Copy Tags
            </button>
          </div>
          <p
            id="toolbox-tag-status"
            style="font-size: 11px; color: var(--sys-text-subtle); min-height: 16px; margin: 6px 0 0 0;"
          ></p>
          <div
            id="toolbox-tag-list"
        ...
        <div class="group-box toolbox-panel">
          ...
          <div class="form-group" style="margin-top: 6px;">
            <button type="button" class="win-button" id="toolbox-ocr-btn">
              <i class="bi bi-fonts"></i> Run OCR
            </button>
            <label
        ...
        <div class="group-box toolbox-panel">
          ...
          <div class="form-group" style="margin-top: 6px;">
            <button type="button" class="win-button" id="toolbox-detect-btn">
          ...
          <p
            id="toolbox-detect-status"
            style="font-size: 11px; color: var(--sys-text-subtle); min-height: 16px; margin: 6px 0 0 0;"
          ></p>
          <div
            id="toolbox-detect-list"
            class="toolbox-output"
            style="display: flex; flex-direction: column; gap: 4px; padding: 4px;"
          ></div>
        </div>
        ...
        <div class="group-box toolbox-panel">
          ...
          <div class="form-group" style="margin-top: 6px; flex-wrap: wrap;">
            <button type="button" class="win-button" id="toolbox-safety-btn">
              <i class="bi bi-shield-check"></i> Classify Safety
            </button>
            <span
              id="toolbox-safety-summary"
              style="display: none; font-size: 11px; font-weight: 600; white-space: nowrap;"
            ></span>
          </div>
          <p
            id="toolbox-safety-status"
            style="font-size: 11px; color: var(--sys-text-subtle); min-height: 16px; margin: 6px 0 0 0;"
          ></p>
    ...
    <div id="toolbox-fullscreen-overlay">
      <button
        type="button"
        class="toolbox-fullscreen-close"
        id="toolbox-fullscreen-close"
        title="Close (Esc)"
      >
        <i class="bi bi-x-lg"></i>
      </button>
