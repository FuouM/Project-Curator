...
function processLogLine(line: string): string {
  ...
  let escaped = cleanLine.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
  ...
  containers.forEach((container) => {
    ...
    const toggle = (expand: boolean) => {
      ...
      if (expand) {
        ...
        footer.innerHTML =
          '<i class="bi bi-chevron-bar-up" style="margin-right: 4px;"></i>Collapse Block';
      } else {
    ...
    <div class="group-box" style="display: flex; flex-direction: column; flex: 1; min-height: 0;">
      ...
      <div style="display: flex; gap: 8px; margin-bottom: 8px; align-items: center;">
        <div style="display: flex; gap: 0;">
          <button
            class="win-button log-tab active"
            id="log-tab-dashboard"
            style="border-radius: 2px 0 0 2px;"
          >
            Dashboard
          </button>
          <button
            class="win-button log-tab"
            id="log-tab-service"
        ...
        <button class="win-button" id="open-log-file-btn">
          <i class="bi bi-folder2-open"></i> Open File Location
        </button>
        <button class="win-button" id="refresh-logs-btn">
      ...
      <div
        id="log-content"
        style="flex: 1; font-family: 'Consolas', monospace; font-size: 11px; background-color: #1e1e1e; color: #cccccc; border: 1px solid #7a7a7a; padding: 8px; overflow-y: auto; white-space: pre-wrap; word-break: break-all;"
      ></div>
    </div>
  `;
}
