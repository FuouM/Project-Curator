    ...
    if quant_format == "fp16":
        ...
        try:
            ...
            for ipt in model.graph.input:
                if ipt.type.HasField("tensor_type") and ipt.type.tensor_type.elem_type == 10:
                    has_fp16 = True
                    break
            if not has_fp16:
            ...
            if has_fp16:
                print("Model contains FP16 components. Upcasting to FP32 before dynamic quantization...")
                # Convert inputs
                for ipt in model.graph.input:
                    if ipt.type.HasField("tensor_type") and ipt.type.tensor_type.elem_type == 10:
                ...
                for opt in model.graph.output:
                    if opt.type.HasField("tensor_type") and opt.type.tensor_type.elem_type == 10:
                        opt.type.tensor_type.elem_type = 1

                # Convert value_info
                for vi in model.graph.value_info:
                    if vi.type.HasField("tensor_type") and vi.type.tensor_type.elem_type == 10:
                ...
                for node in model.graph.node:
                    ...
                    for attr in node.attribute:
                        if attr.type == onnx.AttributeProto.TENSOR and attr.t.data_type == 10:
                            ...
                            arr_f32 = arr.astype(np.float32)
                            new_tensor = numpy_helper.from_array(arr_f32, name=attr.t.name)
