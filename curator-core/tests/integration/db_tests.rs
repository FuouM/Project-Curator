...
async fn test_cascade_deletions_and_foreign_keys() {
    ...
    let (tag_count,): (i64,) =
        sqlx::query_as("SELECT COUNT(*) FROM image_tags WHERE image_id = 10")
            .fetch_one(&pool)
            .await
            .unwrap();
    ...
    let (meta_count,): (i64,) =
        sqlx::query_as("SELECT COUNT(*) FROM image_parsed_metadata WHERE image_id = 10")
            .fetch_one(&pool)
            .await
            .unwrap();
    ...
    let (det_count,): (i64,) =
        sqlx::query_as("SELECT COUNT(*) FROM character_detections WHERE image_id = 10")
            .fetch_one(&pool)
            .await
            .unwrap();
    ...
    assert_eq!(
        tag_count_after, 0,
        "image_tags was not cleaned up on image deletion"
    );

    let (meta_count_after,): (i64,) =
        sqlx::query_as("SELECT COUNT(*) FROM image_parsed_metadata WHERE image_id = 10")
            .fetch_one(&pool)
            .await
            .unwrap();
    assert_eq!(
        meta_count_after, 0,
        "image_parsed_metadata was not cleaned up on image deletion"
    );

    let (det_count_after,): (i64,) =
        sqlx::query_as("SELECT COUNT(*) FROM character_detections WHERE image_id = 10")
            .fetch_one(&pool)
            .await
            .unwrap();
    assert_eq!(
        det_count_after, 0,
        "character_detections was not cleaned up on image deletion"
    );
}
...
async fn test_tag_taxonomy_and_conflict_resolution() {
    ...
    assert_eq!(
        res.rows_affected(),
        0,
        "Duplicate tag insertion should affect 0 rows with ON CONFLICT DO NOTHING"
    );
...
async fn test_character_identities_and_detections_crud() {
    ...
    let (assigned_id,): (Option<i64>,) =
        sqlx::query_as("SELECT identity_id FROM character_detections WHERE id = 10")
            .fetch_one(&pool)
            .await
            .unwrap();
...
fn test_ocr_detector_sanity() {
    let detector = curator_core::OcrDetector::new(
        "../.curator/models",
        curator_core::DevicePreference::Cpu,
        false,
        false,
    );
...
fn test_ocr_image_transcription_extraction() {
    ...
    let ocr_files = [
        (
            "pp-ocrv6-medium/det/inference.onnx",
            "PP-OCRv6_medium_det_onnx/inference.onnx",
        ),
        (
            "pp-ocrv6-medium/det/inference.yml",
            "PP-OCRv6_medium_det_onnx/inference.yml",
        ),
        (
            "pp-ocrv6-medium/rec/inference.onnx",
            "PP-OCRv6_medium_rec_onnx/inference.onnx",
        ),
        (
            "pp-ocrv6-medium/rec/inference.yml",
            "PP-OCRv6_medium_rec_onnx/inference.yml",
        ),
    ];
    ...
    let img_path = workspace_root
        .join("assets")
        .join("test_images")
        .join("1533999207878.png");
    if !img_path.exists() {
        return;
    }
    ...
    let img = curator_core::image::ImageBuffer::<curator_core::image::Rgb<u8>, Vec<u8>>::from_raw(
    ...
    println!(
        "Extracted {} text blocks, {} bubble detections:",
        results.len(),
        bubbles.len()
    );
    for (idx, det) in results.iter().enumerate() {
        println!(
            "[{}] \"{}\" (conf: {:.2}, bubble: {})",
            idx, det.text, det.confidence, det.is_from_bubble
        );
    }

    assert!(
        !results.is_empty(),
        "No texts extracted from the reference image"
    );
    let joined_texts = results
        .iter()
        .map(|d| d.text.to_uppercase())
        .collect::<Vec<_>>()
        .join(" ");
    assert!(
        joined_texts.contains("FREEDOM")
            || joined_texts.contains("DILEMMA")
            || joined_texts.contains("BOCCHI")
            || joined_texts.contains("PRETTY"),
        "Expected OCR results to contain key transcription keywords"
    );
}
